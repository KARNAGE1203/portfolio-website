...
const LOCATIONS = [
  [77.2, 28.6],   [- 123.1, 49.0], [55.3, 25.2],   [-74.0, 40.7],
  [-0.1, 51.5],   [139.7, 35.7],   [151.2, -33.9],  [-46.6, -23.5],
  [103.8, 1.35],  [31.2, 30.0],    [-118.2, 34.0],  [2.35, 48.85],
  [37.6, 55.75],  [36.8, -1.3],
]

const ROUTES = [
  [0,1],[1,2],[2,0],[3,4],[4,11],[5,6],[7,13],
  [8,9],[10,5],[12,2],[9,4],[6,8],[3,7],[13,0],
]
...
function FlightPath({ d, index }) {
  ...
  useEffect(() => {
    ...
    setGlowStyle({
      ...
      animationDuration: `${Math.min(10, Math.max(4, len / 90))}s`,
...
export default function WorldMapBackground() {
  ...
  if (!width || !height) return <div className="world-map" aria-hidden="true" />
      ...
      <svg className="world-map__svg" viewBox={`0 0 ${width} ${height}`}>
        ...
        <g className="world-map__routes">
          {ROUTES.map(([a, b], i) => (
            <FlightPath
              key={i}
              d={arcPath(points[a], points[b], i % 2 === 0 ? 1 : -1)}
              index={i}
            />
          ))}
        </g>
